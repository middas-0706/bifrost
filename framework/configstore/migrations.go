...
type rateLimitOwnerColumnSpec struct {
	fieldName  string
	columnName string
	indexName  string
}

func rateLimitOwnerColumnSpecs() []rateLimitOwnerColumnSpec {
	return []rateLimitOwnerColumnSpec{
...
func createRateLimitOwnerIndexes(ctx context.Context, db *gorm.DB) error {
	db = db.WithContext(ctx)
	if db.Dialector.Name() == "postgres" {
		for _, spec := range rateLimitOwnerColumnSpecs() {
			stmt := fmt.Sprintf("CREATE INDEX CONCURRENTLY IF NOT EXISTS %s ON governance_rate_limits (%s)", spec.indexName, spec.columnName)
			if err := db.Exec(stmt).Error; err != nil {
				return fmt.Errorf("failed to create concurrent index on governance_rate_limits.%s: %w", spec.columnName, err)
			}
		}
		return nil
	}
...
func migrationAddRateLimitOwnerColumns(ctx context.Context, db *gorm.DB) error {
	m := migrator.New(db, migrator.DefaultOptions, []*migrator.Migration{{
		...
		Rollback: func(tx *gorm.DB) error {
			...
			migrator := tx.Migrator()
			for _, spec := range rateLimitOwnerColumnSpecs() {
				_ = tx.Exec(fmt.Sprintf("DROP INDEX IF EXISTS %s", spec.indexName)).Error
			}
	...
	if err := createRateLimitOwnerIndexes(ctx, db); err != nil {
		return err
	}
	return nil
}
